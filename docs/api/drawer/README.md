...
| Stack | Mechanism |
| --- | --- |
| React / Vue | Ark **Presence**; `lazyMount`, `unmountOnExit`, and `skipAnimationOnMount` are forwarded. Theme CSS animates `[data-state=open\|closed]`. Presence waits for `animationend`, then fires `exitcomplete` for Zag’s `closing` state. |
