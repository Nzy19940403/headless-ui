...
export const componentApis = {
  ...
  'tree-select': {
    contract: 'TreeSelectContract',
    contractFile: 'packages/core/src/tree-select-contract.ts',
    props: [
      { name: 'nodes', type: 'TreeNodeContract[]', description: 'Hierarchical options: id, label, children?' },
      { name: 'value', type: 'string | string[]', description: 'Controlled selected node id(s)' },
      { name: 'defaultValue / default-value', type: 'string | string[]' },
      { name: 'placeholder', type: 'string', defaultValue: "'Select'" },
      { name: 'selectBranches', type: 'boolean', defaultValue: 'false', description: 'Allow selecting branch nodes' },
      { name: 'multiple', type: 'boolean', defaultValue: 'false', description: 'Keep open and toggle multiple values' },
      { name: 'columnWidth', type: 'number | string', defaultValue: '180', description: 'Width of every cascader column' },
      { name: 'columnWidths', type: 'Array<number | string>', description: 'Optional width per cascader depth' },
      { name: 'defaultExpandedValue / expandedValue', type: 'string[]' },
      { name: 'height', type: 'number | string', defaultValue: '240' },
    ...
    events: [
      { name: 'value-change', type: '{ value: string | string[]; selectedValue: string[] }' },
      { name: 'expanded-change', type: '{ expandedValue: string[] }' },
    ],
    eventMapping: {
      react: 'onValueChange / onExpandedChange',
      vue: 'value-change / expanded-change / update:value',
      webComponent: 'onValueChange / onExpandedChange props + kebab CustomEvents',
    },
    notes: [
      'Branch nodes expand on click; leaf-only selection unless selectBranches.',
      'columnWidth (all columns) + columnWidths[depth] (optional per column).',
      'Custom content: React renderNode/renderValue/renderTag; Vue #node #value #tag; WC renderNode property or <template data-slot="node|value|tag"> with [data-bind].',
    ],
  },
  radio: {
  ...
  tag: {
    ...
    props: [
      ...
      { name: 'size', type: "'sm' | 'md' | 'lg'", defaultValue: "'md'" },
      { name: 'content', type: 'string', description: 'Used when children / slot content is not provided' },
  ...
  segmentGroup: {
    ...
    props: [
      { name: 'items', type: '{ value; label; disabled? }[]', description: 'Required; item.disabled disables one segment' },
      { name: 'value / defaultValue', type: 'string' },
      { name: 'label / name', type: 'string' },
      { name: 'disabled', type: 'boolean', description: 'Disable entire group' },
      {
        name: 'fullWidth / full-width',
        type: 'boolean',
        defaultValue: 'false',
        description: 'false = hug content; true = 100% parent width, equal segments',
      },
      { name: 'size', type: "'sm' | 'md' | 'lg'", defaultValue: "'md'" },
    ],
    events: [{ name: 'value-change', type: '{ value: string }' }],
    ...
    notes: [
      'Default layout hugs content (not full-bleed). Pass fullWidth to stretch.',
      'Vue: Core value maps to Ark modelValue.',
    ],
  },
  datePicker: {
  ...
  table: {
    ...
    props: [
      { name: 'columns', type: 'TableColumnContract[]', description: 'accessorKey, header, cellType, align, size/minSize/maxSize, pinned, enableSorting/Resizing/Ordering' },
      { name: 'data', type: 'Record<string, unknown>[]' },
      { name: 'fillHeight', type: 'boolean', defaultValue: 'false' },
      ...
      { name: 'pageSize / pagination / defaultPagination', type: 'number | { pageIndex; pageSize }' },
      { name: 'padEmptyRows', type: 'boolean', defaultValue: 'true when paginated' },
      { name: 'density', type: "'comfortable' | 'compact'", defaultValue: "'comfortable'" },
      { name: 'resizeable / columnResizeMode / columnSizing', type: 'boolean | onChange|onEnd | Record<string, number>' },
      { name: 'draggable / columnOrder', type: 'boolean | string[]' },
      { name: 'enableExpanding / expanded / defaultExpanded', type: 'boolean | ExpandedState' },
      { name: 'emptyText / caption / loading', type: 'string / boolean' },
      { name: 'renderExpanded / getRowCanExpand / columnDefs', type: 'React escape hatches', description: 'Not in Core; React adapter' },
    ],
    events: [
      { name: 'sorting-change', type: '{ sorting }' },
      { name: 'pagination-change', type: '{ pagination }' },
      { name: 'column-sizing-change', type: '{ sizing }', description: 'React when resizeable' },
      { name: 'column-order-change', type: '{ order }', description: 'React when draggable' },
      { name: 'expanded-change', type: '{ expanded }', description: 'React when expanding' },
    ],
    eventMapping: {
      ...
      webComponent: 'same property names + kebab CustomEvents',
    },
    notes: [
      'cellType: text | number | tag | badge | progress | datetime | boolean.',
      'pinned left/right; resizeable; draggable column order (React dnd-kit, Vue/WC HTML5 DnD).',
      'expand: React renderExpanded; Vue #expanded; WC renderExpanded prop or template data-slot=expanded.',
    ],
  },
  tree: {
  ...
  navMenu: {
    ...
    contractFile: 'packages/core/src/nav-menu-contract.ts',
    props: [
      { name: 'items', type: 'NavMenuItemContract[]', description: 'Nested items, groups and dividers.' },
      { name: 'mode', type: "'vertical' | 'horizontal' | 'inline'", defaultValue: "'vertical'" },
      { name: 'theme', type: "'light' | 'dark'", defaultValue: "'light'" },
      { name: 'selectable / multiple', type: 'boolean', defaultValue: 'true / false' },
      { name: 'inlineCollapsed', type: 'boolean', defaultValue: 'false' },
      { name: 'inlineIndent', type: 'number', defaultValue: '24' },
      { name: 'selectedKeys / defaultSelectedKeys', type: 'string[]' },
      { name: 'openKeys / defaultOpenKeys', type: 'string[]' },
      { name: 'triggerSubMenuAction', type: "'hover' | 'click'", defaultValue: "'click' in inline mode; otherwise 'hover'" },
    ],
    events: [
      { name: 'onSelect', type: '{ key; keyPath; selectedKeys; item }' },
      { name: 'onDeselect', type: '{ key; keyPath; selectedKeys; item }', description: 'Only used with multiple.' },
      { name: 'onOpenChange', type: '{ openKeys }' },
    ],
    notes: [
      'React implementation is first; HDrawer/HSplit are composed outside HNavMenu.',
      'React-only renderLabel supports custom route labels and content.',
      'Menu state is controlled/uncontrolled like Ant Design Menu.',
    ],
  },
  chart: {
